...
public class Login extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
       ...
       Query q=s.createQuery("Select u From Utilisateur u where u.email=:login and u.mdp=:mdp");
       q.setString("login", login);
       q.setString("mdp", mdp);
       List<Utilisateur> l=q.list();
       if(l.size()>0){
           if(l.get(0).getEtatcpt()){
                    request.getSession().setAttribute("user", l.get(0));
                    request.getRequestDispatcher("/views/profile.jsp").forward(request, response); 
           }else{
                    request.setAttribute("msg","this account is deactivated! ");
